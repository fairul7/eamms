 ...
 */
public class ArchiveContentObjectPanel extends Panel {
    ...
    public ArchiveContentObjectPanel() {
    }

    public ArchiveContentObjectPanel(String name) {
        super(name);
    }

    public String getDefaultTemplate() {
        return "cms/admin/archiveContentObjectPanel";
    }
    ...
    protected void addContainerForm() {
        try {
            if (getId() != null) {
                ...
                // add buttons
                archiveButton = new Button("archiveButton");
                archiveButton.setText(application.getMessage("general.label.archive", "Archive"));
                containerForm.addChild(archiveButton);
                unarchiveButton = new Button("unarchiveButton");
                unarchiveButton.setText(application.getMessage("general.label.unarchive", "Unarchive"));
                containerForm.addChild(unarchiveButton);
                ...
                containerForm.addFormEventListener(new FormEventAdapter() {
                    public Forward onValidate(Event evt) {
                        ...
                        if (archiveButton.equals(button)) {
                            ContentObject contentObject = contentObjectView.getContentObject();
                            boolean recursive = recursiveBox.isChecked();
                            archiveContentObject(contentObject, recursive);
                            init();
                            return new Forward("archive");
                        }
                        else if (unarchiveButton.equals(button)) {
                            ContentObject contentObject = contentObjectView.getContentObject();
                            boolean recursive = recursiveBox.isChecked();
                            unarchiveContentObject(contentObject, recursive);
                            init();
                            return new Forward("unarchive");
                        }
                        else {
    ...
    protected void archiveContentObject(ContentObject contentObject, boolean recursive) {
        Application application = Application.getInstance();
        ContentManager contentManager = (ContentManager)application.getModule(ContentManager.class);
        try {
            // save
            User user = getWidgetManager().getUser();
            contentManager.archive(contentObject.getId(), recursive, user);
        }
        catch (Exception e) {
            throw new RuntimeException(e.toString());
        }
    ...
    protected void unarchiveContentObject(ContentObject contentObject, boolean recursive) {
        Application application = Application.getInstance();
        ContentManager contentManager = (ContentManager)application.getModule(ContentManager.class);
        try {
            // save
            User user = getWidgetManager().getUser();
            contentManager.unarchive(contentObject.getId(), recursive, user);
        }
        catch (Exception e) {
            throw new RuntimeException(e.toString());
        }
