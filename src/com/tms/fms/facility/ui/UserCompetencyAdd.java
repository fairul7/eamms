...
public class UserCompetencyAdd extends Form
{
    ...
    public Forward onValidate(Event event)
    {
        ...
        if(add.getAbsoluteName().equals(findButtonClicked(event)))
        {
            ...
            if(!(DEFAULT_SELECTION_VALUE.equals(selectedType) || DEFAULT_SELECTION_VALUE.equals(selectedName) || selectedLevel == null || "".equals(selectedLevel)))
            {
                ...
                {
                	//get Opener 
                	UserCompetencyForm opener=(UserCompetencyForm)getWidgetManager().getWidget(redirectTo);
                    Map competencies = opener.getCompetencies();
                    
                    Competency competency = handler.getCompetency(selectedName);
                    competency.setCompetencyLevel(selectedLevel);
                    //if(competencies.containsKey(arg0)){
                    	competencies.put(competency.getCompetencyId(),competency);
                    //}
                    opener.setCompetencies(competencies);
                    ...
                    init();
                    	forward = new Forward("forwardToFMS");
	...
	public void onRequest(Event event)
	{
		...
		if(event.getRequest().getParameter("redirectTo")!=null){
			redirectTo=event.getRequest().getParameter("redirectTo");
		}
