                    ...
                    <c:choose>
                        <c:when test="${!(empty widget.forums)}">
                            ...
                            <c:forEach items="${widget.forums}" var="forum">
                                <tr>
                                    <td class="forumRow"><fmt:message key='forum.label.forum'/>:
<%--
                                        <x:event name="${widget.absoluteName}" type="${forum_event}" param="${forum_key}=${forum.forumId}">
                                            <b><c:out value="${forum.name}"/></b>
                                        </x:event>
--%>
                                        <a href="<c:url value="/ekms/forums/forumTopicList.jsp"/>?<c:out value="${forum_key}=${forum.forumId}"/>">
                                            <b><c:out value="${forum.name}"/></b>
                                        </a>
                                    </td>
                                </tr>
...
--%>
                                                <a href="<c:url value="/ekms/forums/forumMessageList.jsp"/>?<c:out value="${thread_key}=${thread.threadId}&type=${thread.type}&${message_key}=${thread.id}"/>">
                                                    <c:out value="${thread.subject}"/>
                                                </a>
                                                (<fmt:formatDate value="${thread.modificationDate}" pattern="${globalDatetimeLong}"/>) - <c:out value="${thread.ownerId}"/>
                                            </li>
                                        </td>
                                    </tr>
                                </c:forEach>
                        ...
                        <c:otherwise>
                            <tr><td class="forumRow"><fmt:message key='forum.label.noForums'/></td></tr>
                        </c:otherwise>
                    </c:choose>
                </table>
            </td>
