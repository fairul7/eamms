    ...
    <tr>
        <td>
                        ...
                        <td align="right">
                            <c:forEach items="${model.actionList}" var="action" varStatus="hdrStatus">
                                <c:if test="${!empty action.message}">
                                    <c:set var="onclick"><c:out escapeXml="false" value="${action.message}"/></c:set>
                                </c:if>
                                <c:if test="${empty action.message}">
            ...
            <table border="0" cellpadding="2" cellspacing="1" width="100%">
                ...
                <script>
                <!--
                	function checkOnlyOneCheckboxSelectected() {
                		obj = document.forms['<c:out value="${table.rootForm.absoluteName}"/>']['<c:out value="${table.tableRowName}"/>'];
                		count = 0;
                		...
                		if (count != 1) {
                			alert('<fmt:message key="messaging.filtering.msg.selectOneToMoveUpOrDown" />');
                			return false;
                		}
                	}
