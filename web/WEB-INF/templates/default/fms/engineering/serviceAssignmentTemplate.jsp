...
<table align="center" class="borderTable" cellpadding="3" cellspacing="1" width="95%">
	<tr>
		...
		<th width="15%"><fmt:message key="fms.facility.label.assignmentId"/></th>
		...
		<th width="20%"><fmt:message key="fms.facility.label.facilityAssigned"/></th>
						...
						<c:if test="${obj.statusLabel eq 'New' or obj.statusLabel eq 'Assigned' or obj.statusLabel eq 'Cancelled'}">
							...
							<c:choose>
								<c:when test="${obj.cancelBy eq null}" >
									...
									<c:if test="${obj.rateCardCategoryId eq '' || obj.rateCardCategoryId eq null}">
									...
									<c:if test="${obj.competencyId eq '' || obj.competencyId eq null}">
										<input type="hidden" id="facilityId_<c:out value="${obj.assignmentId}"/>" name="facilityId_<c:out value="${obj.assignmentId}"/>" value="<c:out value="${obj.rateCardCategoryId}"/>" />
									</c:if>
