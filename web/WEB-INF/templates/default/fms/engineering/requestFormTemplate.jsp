...
<table class="profileTable" cellpadding="3" cellspacing="1" width="100%">
    ...
    <tr id="program" <c:if test="${widget.lbRequestType != 'I'}">style="display:none"</c:if>>
 	...
 	<tr>
        <td width="30%" nowrap class="profileRow" valign="top" align="right"><b><fmt:message key="fms.request.label.servicesRequired"/>*</b>&nbsp;</td>
        <td width="70%" class="profileRow">
	        <c:if test="${! empty widget.services}">
	        	<table >
			        <c:forEach items="${widget.services}" var="service" varStatus="sno">
			        	<c:if test="${(sno.index%2)==0}">
			        		<tr>
			        	</c:if>
			            	<td><x:display name="${service.absoluteName}"/></td>
			            <c:if test="${(sno.index%2)==1}">
			        		</tr>
			        	</c:if>
			        </c:forEach>
	    		</table>
    ...
    <tr>
        <td width="30%" nowrap class="profileRow" align="right"><b><fmt:message key="fms.request.label.dateRequiredFrom"/>*</b>&nbsp;</td>
        <td width="70%" class="profileRow"><x:display name="${widget.requiredFrom.absoluteName}"/></td>
    </tr>
    <tr>
        <td width="30%" nowrap class="profileRow" align="right"><b><fmt:message key="fms.request.label.dateRequiredTo"/>*</b>&nbsp;</td>
        <td width="70%" class="profileRow"><x:display name="${widget.requiredTo.absoluteName}"/></td>
    </tr>
...
<script>
	...
	function check_cb_valid()
	{
	    if(document.getElementsByName('<c:out value="${widget.absoluteName}"/>.services')[0].checked)
	    {
	    	document.getElementsByName('<c:out value="${widget.absoluteName}"/>.services')[4].disabled = true;
	    }
	    else
	    {
	    	document.getElementsByName('<c:out value="${widget.absoluteName}"/>.services')[4].disabled = false;
	    }
	    
	    if(document.getElementsByName('<c:out value="${widget.absoluteName}"/>.services')[4].checked)
	    {
	    	document.getElementsByName('<c:out value="${widget.absoluteName}"/>.services')[0].disabled = true;
	    }
	    else
        {
            document.getElementsByName('<c:out value="${widget.absoluteName}"/>.services')[0].disabled = false;
        }
	}
