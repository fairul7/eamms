<%@ page import="kacang.runtime.*,kacang.ui.*, kacang.stdui.Table, com.tms.fms.widgets.ExtendedTableColumn" %>
...
<%!
    public String getAlign(PageContext pageContext, String field) {
        try {
            ExtendedTableColumn column = (ExtendedTableColumn) pageContext.getAttribute(field);
    ...
    public String getHeaderAlign(PageContext pageContext, String field) {
        try {
            ExtendedTableColumn column = (ExtendedTableColumn) pageContext.getAttribute(field);
            if (column.getHeaderAlign() != null) {
                return column.getHeaderAlign();
            } else if (column.getAlign() != null) {
                return column.getAlign();
            }
        } catch (Exception e) {
        }
    ...
    public String getStyle(PageContext pageContext, String field) {
        try {
            ExtendedTableColumn column = (ExtendedTableColumn) pageContext.getAttribute(field);
        ...
        <td>
            ...
            <c:if test="${!empty model.actionList[0]}">
                <table border="0" cellpadding="2" cellspacing="4" class="tableBackground" width="100%">
                    <tr>
                        <td align="right">
                            <input class="tableButton" type="button" value="<fmt:message key="fms.setup.btnAddAbw"/>" onClick="newAbwCode();"/>
            ...
            <table border="0" cellpadding="2" cellspacing="1" width="100%">
                ...
                <%-- Show Headers --%>
                <thead>
                    <c:if test="${not empty(requestScope.extendedtable_header)}">
                        <c:out value="${requestScope.extendedtable_header}" escapeXml="false"/>     
                    </c:if>
                    ...
                    <tr class="tableHeader">
                        ...
                        <c:forEach items="${model.columnList}" var="column" varStatus="hdrStatus">
                            <% String headerAlign = getHeaderAlign(pageContext, "column"); %>
                            <td align="<%= headerAlign %>" height="25">
                    ...
                    <c:forEach items="${model.tableRows}" var="row" varStatus="status">
                        <tr class="tableRow">
                            ...
                            <c:forEach items="${model.columnList}" var="column">
                                <% String align = getAlign(pageContext, "column"); %>
                                <% String style = getStyle(pageContext, "column"); %>
                                <td align="<%= align %>" style="<%= style %>" valign="top" height="25">
                                    <c:choose>
                ...
                <%-- Show Paging --%>
                <tfooter>
                    <c:if test="${not empty(requestScope.extendedtable_footer)}">
                        <c:out value="${requestScope.extendedtable_footer}" escapeXml="false"/>     
                    </c:if>
                    <tr class="tableBackground">
                        <td colspan="<c:out value="${colspan}"/>" align="right">&nbsp;
                        <c:if test="${empty(requestScope.extendedtable_hidePaging)}">
