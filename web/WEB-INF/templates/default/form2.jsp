			...
			<tr>
				<%
					for (Iterator i = form.getChildren().iterator(); i.hasNext();)
					{
						...
						%>
                        <% if(!(child instanceof Hidden)) { %>
                        	<%if ("effectiveDays".equals(child.getName())) { 
                        	      kacang.stdui.Label _l = (kacang.stdui.Label) child;
                        	      if (_l.getText() != null && _l.getText().trim().length() > 0) {
                        	%>
                        	...
                        	<%   } 
                        	   } else if("errorMsg".equals(child.getName())) { %>
                            	<td class="classRow">
                            		<x:display name="<%= child.getAbsoluteName() %>" />
                            	</td>	
                        	<% } else { %>
                            <td
                            <% 
                            	if ( !("errorMsg".equals(child.getName())) && 
                            	 !("effectiveDays".equals(child.getName())) &&
                            	child instanceof kacang.stdui.Label) { %>
                            	class="classRowLabel"
                            	align="right"
                            <% } else { %>
                            	class="classRow"
