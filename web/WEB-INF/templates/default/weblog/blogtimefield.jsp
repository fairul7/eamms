...
<c:forEach begin="0" end="59" step="1" var="m">
    <option value="<c:out value="${m}"/>" <c:if test="${m eq timefield.minute}">selected</c:if>>
        <fmt:formatNumber pattern="00" value="${m}"/></option>
</c:forEach>
</select>

<select name="<c:out value="${timefield.secondName}"/>">
<c:forEach begin="0" end="59" step="1" var="s">
    <option value="<c:out value="${s}"/>" <c:if test="${s eq timefield.second}">selected</c:if>>
        <fmt:formatNumber pattern="00" value="${s}"/></option>
</c:forEach>
</select>
