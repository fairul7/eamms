...
<tr>
    <td>--%>
        ...
        <FONT COLOR="#FF0000"><fmt:formatDate value="${task.dueDate}" pattern="${globalDatetimeLong}"/> </FONT>
        |  <!--<FONT SIZE="2" CLASS="calendarContentFont">--><B><fmt:message key='taskmanager.label.Category'/>: </b><!--</font>--><c:out value="${task.category}"  /><BR>
      ...
      <c:when test="${!task.hidden}" >
       ...
       %>
        ...
        <c:if test="${canEdit}" >
           ...
           <c:if test="${task.isproject=='Yes'}">  
           <input value="<fmt:message key="calendar.label.buttonEdit"/>" type="button" class="button" onClick="window.open('/ekms/worms/task.jsp?taskId=<c:out value="${task.eventId}" />', 'taskWindow', 'height=450,width=550,left=50,top=50,screenx=50,screeny=50,scrollbars=yes,resizable=yes')" />                   
           </c:if>
          ...
          <c:url var="deleteUrl" value="${eventUrl}">
           ...
           <input value="<fmt:message key="calendar.label.buttonDelete"/>" type="button" class="button" onClick="if(confirm('<fmt:message key="messaging.label.confirmDelete"/>')){document.location='<c:out value="${deleteUrl}"/>';}"/>
        ...
        <c:if test="${widget.thisAssignee.taskStatus == notstarted}" >
          <c:url var="startUrl" value="/ekms/taskmanager/setDateForm.jsp"> <%--${eventUrl}">--%>
                <c:param name="${cn}" value="${widget.absoluteName}"/>
        ...
        <%-- <INPUT TYPE="button" value="<fmt:message key="calendar.label.buttonStart"/>" class="button" onClick="document.location = '<c:out value="${startUrl}"/>';"/> --%>
            <input type="button" value="<fmt:message key="calendar.label.buttonStart"/>" class="button" onClick="window.open('<c:out value="${startUrl}"/>','startTaskForm','height=300,width=400,left=50,top=50,screenx=50,screeny=50,scrollbars=yes,resizable=yes')">
        </c:if>


        <c:if test="${canComplete}" >
          <c:url var="completeUrl" value="/ekms/taskmanager/setDateForm.jsp"><%-- ${eventUrl}"> --%>
        <c:param name="${cn}" value="${widget.absoluteName}"/>
        <c:param name="${et}" value="complete"/>
        <c:param name="taskId" value="${task.eventId}"/>
              <c:param name="taskEventType" value="completed"/>
    </c:url>
        <%-- <INPUT TYPE="button" value="<fmt:message key="calendar.label.buttonCompleted"/>" class="button" onClick="document.location = '<c:out value="${completeUrl}"/>';"/> --%>
            <INPUT TYPE="button" value="<fmt:message key="calendar.label.buttonCompleted"/>" class="button" onClick="window.open('<c:out value="${completeUrl}"/>','taskCompletionForm','height=300,width=400,left=50,top=50,screenx=50,screeny=50,scrollbars=yes,resizable=yes')"/>
        </c:if>
