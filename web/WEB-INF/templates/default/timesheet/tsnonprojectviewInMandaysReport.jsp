...
function openPrintableViewWindow(isSuccess) {
	if(isSuccess) {
		...
		mywindow = window.open('printNonTimeSheetMandaysReport.jsp?projectid=<c:out value='${w.projectId}'/>&startDate=' + dateTimeForm['<c:out value="${widget.fromDate.absoluteName}"/>'].value + '&endDate=' + dateTimeForm['<c:out value="${widget.toDate.absoluteName}"/>'].value, 'mandaysReportPrintableView', 'resizable=1, scrollbars=yes, statusbar=1');
		mywindow.moveTo(0,0);
	}
}
...
<tr class="contentBgColor">
    <td>
    	<strong><fmt:message key="timesheet.label.project"/>:</strong> <c:out value="${w.category.name}"/>
    </td>
    <td align="right">
