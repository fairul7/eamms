...
<!--
    function popupSelectBoxOpen(absoluteName) {
        ...
        window.open('<c:url value="/ekms/purchaseordering/popupFormWindowSingle.jsp"/>?popupSelectBoxCn=' + absoluteName + '&popupFormWindowCn=' + formCn, "popupSelectBoxWindow", "height=400,width=600,status=yes,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=yes");
        return false;
    }
...
--%>
        ...
        var temp = getURLParam("flag");
        //customize: so that the window is reload to go into onRequest()
        if(temp == "new"){
         	window.location = "/ekms/purchaseordering/addSupplier.jsp";
        }else if(temp == "new1"){
        	window.location = "/ekms/purchaseordering/addSupplierToItem.jsp";
        }
