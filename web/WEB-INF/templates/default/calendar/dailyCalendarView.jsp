...
<%= response.encodeURL(request.getRequestURI()) %>?<%= Event.PARAMETER_KEY_WIDGET_NAME %>=<c:out value="${calendar.absoluteName}"/>&<%= Event.PARAMETER_KEY_EVENT_TYPE %>=accept
 ...
 <table BORDER="0" CELLSPACING="1" CELLPADDING="3" width="100%">
        <tr><td class="calendarHeader" ><fmt:message key='calendar.label.calendar'/> > <fmt:message key='calendar.label.dailyCalendarView'/></td></tr>
  ...
  %>
               ...
               <%--     <FONT SIZE="2" CLASS="calendarContentFont">--%>
                    ...
                    <B><fmt:message key='calendar.label.location'/>:</B> <c:out value="${event.location}" />
                     | </c:if>
                     <B><fmt:message key='calendar.label.scheduledby'/>:</B> <c:out value="${event.userName}" /> <c:if test="${event.ownerExcluded}">(<fmt:message key='calendar.label.self-excluded'/>)</c:if>
                    <br>
                    <%--<c:if test="${!event.personal}" >--%>
                        <%-- customisation on 2005-07-13 - list attendees by status --%>
                        <%
        ...
        <B><FONT COLOR="#003366"><fmt:message key='calendar.label.toDoList'/></FONT></B>
         <UL>
            ...
            <%i++;%>
                ...
                <!-- new code: to make task group by category and become expandable -->
                <c:if test="${status.first}">
                    <c:set var="selectedCat" value="${task.task.category}"/>
                </c:if>
                
                <c:choose>
                    <c:when test="${!status.first && !status.last}">
                        <c:if test="${task.task.category != selectedCat}">
                            </div><br/><br/>
                            <b><a href="" onClick="treeToggle('<c:out value="${task.task.category}"/>'); return false;"><c:out value="${task.task.category}"/></a></b>
                            <div id="<c:out value="${task.task.category}"/>" style="display: none">
                            <script>
                            <!--
                                treeLoad('<c:out value="${task.task.category}"/>');
                            //-->
                            </script>
                            <c:set var="selectedCat" value="${task.task.category}"/>
                        </c:if>
...
<%
    String pgID = (String) pageContext.getAttribute("id");
    if(pgID != null){
%>
    <x:display name="${task.absoluteName}" ></x:display>
<%
    }   
%>
<hr size="1" width="70%" align="left">
