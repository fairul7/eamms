		...
		function <c:out value='${textfield.absoluteNameForJavaScript}'/>init() {
			...
			<c:out value='${textfield.absoluteNameForJavaScript}'/>cal.onchange = function() {
                ...
                for(i=0;i<len;i++){
                    ...
                    var index1 = str.indexOf('dueDate');
                    if(index1 != -1){
                        var index2 = str.indexOf('dueDatebutton');
                        if(index2==-1){
                        var manobj = document.forms["<c:out value='${textfield.rootForm.absoluteName}'/>"].elements['<c:out value='${textfield.rootForm.absoluteName}'/>.estimation'];
                        var manTypeObj = document.forms["<c:out value='${textfield.rootForm.absoluteName}'/>"].elements['<c:out value='${textfield.rootForm.absoluteName}'/>.estimationType'];
                        if(manTypeObj.value=='Mandays')
                        manobj.value='1';
                        else if(manTypeObj.value=='Manhours')
                        manobj.value=1*8;
