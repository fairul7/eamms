...
<c:if test="${forward.name == deleted}">
	<script>
		alert("<fmt:message key="emeeting.label.meetingDeleted"/>");
...
<%
    ...
    url += "/calendar" + MeetingFormView.DEFAULT_ACTION_ITEM_PREFIX + ".jsp?" + CalendarView.PARAMETER_KEY_EVENTID + "=";
%>
            ...
            <table width="100%" cellpadding="3" cellspacing="1">
                ...
                <tr>
                    ...
                    <td width="70%" class="emeetingRow">
                        <c:choose>
							<c:when test="${!meeting.event.allDay}" >
								<fmt:formatDate value="${meeting.event.endDate}" pattern="${globalDatetimeLong}"/>
							</c:when>
							<c:otherwise>
								<fmt:formatDate value="${meeting.event.endDate}" pattern="${globalDateLong}"/>
							</c:otherwise>
                        </c:choose>
					</td>
                </tr>
                ...
                <c:if test="${!empty meeting.secretary}" >
                    ...
                    <tr>
                        ...
                        <%
                            String sec = ((String)pageContext.getAttribute("secretary"));
                            if(sec.equals(" -")){
                                pageContext.setAttribute("secretaryName"," - ");
                            }
                            else{
                                User user = UserUtil.getUser((String)pageContext.getAttribute("secretary"));
                                pageContext.setAttribute("secretaryName",user.getName());
                            }
                        %>
                ...
                <%-- customisation on 2005-07-13 --%>
                <% int i=0; boolean isStart=true; %>
                <tr>
                ...
                <c:if test="${! empty widget.relatedMeetings}">
               	...
               	<c:if test="${!widget.parent.hiddenAction}" >
					<tr>
						...
						<td class="emeetingRow" >&nbsp;
							<input type="button" value="<fmt:message key="emeeting.label.print"/>" class="button" onClick="window.open('<c:url value="/ekms/calendar/emeetingPrint.jsp?eventId="/><c:out value="${meeting.eventId}"/>');"/>
							...
							<c:if test="${((widget.userId == meeting.event.userId) || (meeting.secretary == widget.userId)) && (widget.state!=EDIT)}" >
								<input type="button" value="<fmt:message key="emeeting.label.editAgendaItems"/>" onClick="openWindow('<c:out value="${actionItemsUrl}"/>', 'meetingActionWindow');" class="button">
								<input type="button" value="<fmt:message key="emeeting.label.email"/>" class="button" onClick="location= '<c:url value="/ekms/calendar/externalemail.jsp?meetingId=${meeting.eventId}"/>' ;"/>
								<c:if test="${widget.parent.editable}" >
									<input type="button" value="<fmt:message key="calendar.label.buttonEdit"/>" class="button"  onClick="document.location = '<c:url value="/ekms/calendar/editemeetingform.jsp?eventId="/><c:out value="${meeting.eventId}"/>'"/>
								</c:if>
							...
							<c:if test="${meeting.event.attendees != null}">
								...
								<c:forEach items="${meeting.event.attendees}" var="attendee">
									...
									<c:if test="${userId == attendee.userId && attendee.status == att_status && widget.parent.acceptReject}" >
										<input value="<fmt:message key="calendar.label.buttonAccept"/>" type="button" class="button" onClick="document.location = '<c:out value="${acceptUrl}" />&eventId=<c:out value="${meeting.event.eventId}"/>&instanceId=<c:out value="${meeting.event.instanceId}" />&attendeeId=<c:out value="${attendee.attendeeId}"/>';"/>
										<input value="<fmt:message key="calendar.label.buttonReject"/>" type="button" class="button" onClick="window.open('rejectform.jsp?<%= CalendarView.PARAMETER_KEY_EVENTID %>=<c:out value="${meeting.event.eventId}" />&instanceId=<c:out value="${meeting.event.instanceId}" />&attendeeId=<c:out value="${attendee.attendeeId}"/>','rejectform','scrollbars=yes,resizable=yes,width=400,height=200');return false;"/>
									</c:if>
								</c:forEach>
							</c:if>
						</td>
