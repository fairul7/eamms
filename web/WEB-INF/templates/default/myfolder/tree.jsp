...
<script language="javascript" src="<%= request.getContextPath() %>/ekms/myfolder/includes/tree.js">
</script>
...
<tr>
    <td valign=top>
        <c:if test="${!empty root[tree.childrenProperty] && !empty root[tree.childrenProperty][0]}">
            <a style="margin:0px; padding: 2px; text-align:center; width:14px; font-size:16px; line-height: 6px" href="#" onclick="treeToggle('<c:out value="${tree.name}"/><c:out value="${root[tree.displayId]}"/>'); return false">
            	<span id="icon_<c:out value="${tree.name}"/><c:out value="${root[tree.displayId]}"/>">
        ...
        <c:if test="${empty root[tree.childrenProperty] || empty root[tree.childrenProperty][0]}">
            <span style=" margin:0px; padding: 2px; text-align:center; width:14px; font-size:5px; line-height: 6px">
            	<div style="valign:top"><img src="images/minus.JPG" border="0"/></div>
            </span>
        </c:if>
    </td>
    <td>
