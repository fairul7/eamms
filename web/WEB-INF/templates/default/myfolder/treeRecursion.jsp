...
<tr>
    <td valign=top>
        ...
        <c:when test="${empty co[tree.childrenProperty] || empty co[tree.childrenProperty][0]}">
            <span style="margin:0px; padding: 2px; text-align:center; width:14px; font-size:5px; line-height: 6px">
            	<img src="images/minus.JPG" border="0"/>
            </span>
        </c:when>
        <c:when test="${!empty co[tree.childrenProperty] && !empty co[tree.childrenProperty][0] && cookieValue=='none'}">
            <a  style="margin:0px; padding: 2px; text-align:center; width:14px; font-size:16px; line-height: 6px" href="#" onclick="treeToggle('<c:out value="${tree.name}"/><c:out value="${co[tree.displayId]}"/>'); return false">
            	<span id="icon_<c:out value="${tree.name}"/><c:out value="${co[tree.displayId]}"/>">
            		<img src="images/plus.JPG" border="0"/>
            	</span>
            </a>
        </c:when>
        <c:otherwise>
            <a style=" margin:0px; padding: 2px; text-align:center; width:14px; font-size:5px; line-height: 6px" href="#" onclick="treeToggle('<c:out value="${tree.name}"/><c:out value="${co[tree.displayId]}"/>'); return false">
            	<span id="icon_<c:out value="${tree.name}"/><c:out value="${co[tree.displayId]}"/>">
            	<img src="images/minus.JPG" border="0"/>
            	</span>
            </a>
