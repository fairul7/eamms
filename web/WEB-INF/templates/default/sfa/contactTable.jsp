...
<table border="0" cellpadding="4" cellspacing="4" style="border:0px solid gray" width="<c:out value="${table.width}"/>">
    ...
    <td  height="25">
          ...
          <c:choose>
            ...
            <c:when test="${table.desc}">
              <x:event name="${table.absoluteName}" type="sort" param="sort=${prop}" html="class=\"tableHeader\""><c:out value="${hdr}"/></x:event>
            </c:when>
            <c:otherwise>
              <x:event name="${table.absoluteName}" type="sort" param="sort=${prop}&desc=true" html="class=\"tableHeader\""><c:out value="${hdr}"/></x:event>
            </c:otherwise>
          </c:choose>
...
<c:forEach items="${model.tableRows}" var="row" varStatus="status">
    ...
    <%
        ...
        ContactTable ctable= (ContactTable)pageContext.getAttribute("table");
        ctable.setCurrentRow(cRow);
  %>
  <%-- Determine Row Style --%>
<%--
  <c:choose>
    ...
    <c:if test="${showCheckbox}">
        ...
        %>
            ...
            <input
                ...
                <%if(col!=null&&col.contains(pageContext.getAttribute("key").toString())){%>checked<%}%>
            />
