...
<c-rt:set var="eventUrl" value="calendar.jsp"/>
...
<tr>
    <td>--%>
        ...
        | <%--<FONT SIZE="2" CLASS="calendarContentFont">--%><B><fmt:message key='taskmanager.label.Category'/>: </b><%--</font>--%><c:out value="${task.category}"  />
        <%--<c:if test="${! empty task.synchronizationPriority}">
        | <b><fmt:message key="taskmanager.label.priority"/>: </b><c:out value="${task.priorityDescription}"/>
        </c:if>--%>
        <BR>
      ...
      <c:when test="${!task.hidden}" >
       ...
       %>
        ...
        <c:if test="${canReassign}" >
            <INPUT TYPE="BUTTON" CLASS="button" value="<fmt:message key="calendar.label.buttonReassign"/>" onClick="window.open('<c:url value="/kurnia/taskmanager/taskreassignform.jsp?taskId=${task.id}" />','reassignForm','height=400,width=550,left=50,top=50,screenx=50,screeny=50,scrollbars=yes,resizable=yes');" />
        </c:if>
