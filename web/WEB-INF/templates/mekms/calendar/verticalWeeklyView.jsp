...
<c:set var="hasAppointment" value="false" />
 <c:forEach var="appointment" items="${entry.eventList}" varStatus="astatus"  >
    ...
    <td class="verticalCalendarSubheaderWeekend">
              <input class="button" value="New" type="button" onClick="javascript:window.open('/ekms/calendar/appointmentformpop.jsp','addappointment','scrollbars=yes,resizable=yes,width=600,height=380')"/>
              <INPUT TYPE="BUTTON" class="button" value="Prev" onClick="document.location = '<c:out value="${previousUrl}"/>'"/> <INPUT TYPE="BUTTON" class="button" value="Now" onClick="document.location = '<c:out value="${nowUrl}"/>'"/> <INPUT TYPE="BUTTON" class="button" value="Next" onClick="document.location = '<c:out value="${nextUrl}"/>'"/>
    </td>
    </tr>
</table>
