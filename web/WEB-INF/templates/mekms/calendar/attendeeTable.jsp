<%@ page import="kacang.runtime.*,kacang.ui.*, kacang.stdui.Table,
                 com.tms.collab.calendar.ui.MAttendeeTable,
                 java.util.Map,
                 java.util.Iterator,
...
<table border="0" cellpadding="2" cellspacing="1" width="<c:out value="${table.width}"/>">
    <%-- Show Form Header --%>
    ...
    <tr>
        ...
        <tr><td>&nbsp;</td></tr>
        <tr><td class="title"><fmt:message key="calendar.label.CompulsoryAttendees"/></td></tr>
        <tr>
            <td>
                <c:choose>
                    <c:when test="${!empty table.compulsoryMap}">
                        <c:forEach items="${table.compulsoryMap}" var="key">
                            [<x:event name="${table.absoluteName}" type="sel" param="uid=${key.value.id}">x</x:event>]<c:out value="${key.value.name}"/><br>
                        </c:forEach>
                    </c:when>
        ...
        <tr><td class="title"><fmt:message key="calendar.label.Attendees"/></td></tr>
        <tr>
            <td>
                <c:choose>
                    <c:when test="${!empty table.attendeeMap}">
                        <c:forEach items="${table.attendeeMap}" var="key">
                            [<x:event name="${table.absoluteName}" type="sel" param="uid=${key.value.id}">x</x:event>]<c:out value="${key.value.name}"/><br>
                        </c:forEach>
                    </c:when>
                    <c:otherwise>-</c:otherwise>
