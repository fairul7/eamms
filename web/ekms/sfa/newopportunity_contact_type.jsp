...
<x:config>
    <page name="jsp_opportunityTiecontact">
        <com.tms.crm.sales.ui.OpportunityContactTypeForm name="contactTypeForm"  width="100%" />
    </page>
</x:config>

<c:if test="${widgets['jsp_opportunityTiecontact.contactTypeForm'].contactList == null}">
    <x:set name="jsp_opportunityTiecontact.contactTypeForm" property="contactList" value="${widgets['jsp_contactList.table1'].contactList}" />
</c:if>
...
<c:choose>
	...
	<c:otherwise>
		<c:set var="opportunityID" value="${widgets['jsp_opportunityTiecontact.form1'].opportunityID}" />
	</c:otherwise>
</c:choose>
<x:set name="jsp_opportunityTiecontact.form1" property="opportunityID" value="${opportunityID}" />
--%>
<%--
...
<c:if test="${forward.name == 'updatedOpportunityContactType'}" >
       ...
       %>
       <c:redirect url="newopportunity_opportunity_list.jsp?companyID=${companyId}"/>
</c:if>
...
<table cellpadding="4" cellspacing="1" class="sfaBackground" width="100%">
	...
	<x:display name="jsp_opportunityTiecontact.contactTypeForm"/>
