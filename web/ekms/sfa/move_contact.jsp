...
<c:if test="${forward.name == contactsMoved}" >
    <script>
        ...
        location =  "company_view_commain.jsp?companyID=<c:out value="${widgets['jsp_movecontact.table1'].selectedCompanyID}"/>";
    </script>
</c:if>

<!--  x:set name="jsp_movecontact.table1" property="contactList" value="${widgets['jsp_companyviewCommain.contactTable'].contactList}"/ --> 
<x:set name="jsp_movecontact.table1" property="contactList" value="${sessionScope.contactList}" /> 
...
<table cellpadding="4" cellspacing="1" class="sfaBackground" width="100%">
    ...
    <td class="sfaRow">
        ...
        <%
            Collection col = (Collection)request.getSession(true).getAttribute("contactList");
            if(col!=null){
