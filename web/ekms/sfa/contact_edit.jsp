...
<c:if test="${forward.name =='cancel'}" >
    <c:redirect url="/ekms/sfa/contact_details.jsp?contactID=${widgets['jsp_contactEdit.form1'].contactID}" />
...
<c:choose>
	<c:when test="${forward.name == 'contactUpdated'}">
		<script>
		<!--
			location = "contact_details.jsp?contactID=<c:out value="${contactID}"/>";
		//-->
		</script>
	</c:when>
</c:choose>
