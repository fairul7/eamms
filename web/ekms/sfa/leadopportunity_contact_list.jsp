...
<c:if test="${not empty param.clear}" >
    <x:set name="jsp_leadcontactList.table1" property="clear" value="1" />
</c:if>

<c:if test="${not empty param.companyId}" >
    <x:set name="jsp_leadcontactList.table1" property="companyID" value="${param.companyId}" />
    <x:set name="jsp_leadcontactList.form1" property="companyID" value="${param.companyId}" />
</c:if>                    
...
--%>
    ...
    <c:redirect url="leadcontact_view.jsp?contactID=${param.contactID}&backURL=leadopportunity_contact_list.jsp"/>

</c:if>

<c:if test="${forward.name == 'contactAdded'}" >
    <c:set var="table" value="${widgets['jsp_leadcontactList.table1']}"/>
    <c:set var="contactId" value="${widgets['jsp_leadcontactList.form1'].justCreatedID}"/>
    <%
...
<c:if test="${forward.name == 'contactsSelected' }" >
    <c:set var="companyId" value="${widgets['jsp_leadcontactList.table1'].companyID}"/>
    <%
        session.setAttribute("companyId",pageContext.getAttribute("companyId"));
    %>
    <c:redirect url="leadopportunity_contact_type.jsp" />
...
<c:choose>
	...
	<c:otherwise>
		<c:set var="opportunityID" value="${widgets['jsp_leadcontactList.table1'].opportunityID}" />
	</c:otherwise>
</c:choose>
--%>
<%--
<x:set name="jsp_leadcontactList.table1" property="opportunityID" value="${opportunityID}" />
--%>
<%--
<x:set name="jsp_leadcontactList.table1" property="linkUrl" value="contact_view.jsp?opportunityID=${opportunityID}&lulu=lala" />
<x:set name="jsp_leadcontactList.form1" property="opportunityID" value="${opportunityID}" />
--%>

<%--<c:choose>
	<c:when test="${widgets['jsp_leadcontactList.table1'].justTiedContact}">
...
<table cellpadding="4" cellspacing="1" class="sfaBackground" width="100%">
	...
	<x:display name="jsp_leadcontactList.table1"></x:display>
    </td>
    </tr>
   ...
   <table cellpadding="4" cellspacing="1" class="sfaBackground" width="100%">
    ...
    <fmt:message key='sfa.message.newContact'/>
        ...
        <tr>
        <td class="sfaRow">
	<x:display name="jsp_leadcontactList.form1"></x:display>
       </td>
       </tr>
       <tr>
       <td class="sfaFooter">
