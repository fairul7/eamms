...
<jsp:include page="includes/header.jsp" />
	...
	<%
		...
		if (accountManager == null) {
			...
			pageContext.setAttribute("user",dun);	// dun : Deleted User Name
		} else {
			...
			<table border="0" cellpadding="2" cellspacing="2" width="70%" align="center">
				...
				<%
					...
					for (int i=0 ; i < 12 ; i++) {
						...
						opp = om.getMonthlyProjection(temp,userId,String.valueOf(DateUtil.getYear(date))); // Month Projection
						sales = om.getMonthlySales(String.valueOf(month + 1),userId,String.valueOf(DateUtil.getYear(date))); // Monthly Sales
				...
				<tr>
					...
					<td class="report" align="right" bgcolor="#E4EDFC"><a href="view_report_individual.jsp?month=<c:out value="${monthParam}"/>&userId=<%=userId%>&view=monthly"><c:out value="${sales}"/></a></td>
				</tr>
				<%
					}
				%>
...
<%--					<td class="report" align="right"><a href="view_report_individual.jsp?qFrom=<%=sdf2.format(q1From)%>&qTo=<%=sdf2.format(q1To)%>&userId=<c:out value="${user.id}"/>&view=quarterly"><c:out value="${sq1}"/></a></td>--%>
					<td class="report" align="right"><a href="view_report_individual.jsp?qFrom=<%=sdf2.format(q1From)%>&qTo=<%=sdf2.format(q1To)%>&userId=<%=userId%>&view=quarterly"><c:out value="${sq1}"/></a></td>
				</tr>
...
<%--					<td class="report" align="right"><a href="view_report_individual.jsp?qFrom=<%=sdf2.format(q2From)%>&qTo=<%=sdf2.format(q2To)%>&userId=<c:out value="${user.id}"/>&view=quarterly"><c:out value="${sq2}"/></a></td>--%>
					<td class="report" align="right"><a href="view_report_individual.jsp?qFrom=<%=sdf2.format(q2From)%>&qTo=<%=sdf2.format(q2To)%>&userId=<%=userId%>&view=quarterly"><c:out value="${sq2}"/></a></td>
				</tr>
...
<%--					<td class="report" align="right"><a href="view_report_individual.jsp?qFrom=<%=sdf2.format(q3From)%>&qTo=<%=sdf2.format(q3To)%>&userId=<c:out value="${user.id}"/>&view=quarterly"><c:out value="${sq3}"/></a></td>--%>
					<td class="report" align="right"><a href="view_report_individual.jsp?qFrom=<%=sdf2.format(q3From)%>&qTo=<%=sdf2.format(q3To)%>&userId=<%=userId%>&view=quarterly"><c:out value="${sq3}"/></a></td>
				</tr>
...
<%--					<td class="report" align="right"><a href="view_report_individual.jsp?qFrom=<%= sdf2.format(q4From)%>&qTo=<%=sdf2.format(q4To)%>&userId=<c:out value="${user.id}"/>&view=quarterly"><c:out value="${sq4}"/></a></td>--%>
					<td class="report" align="right"><a href="view_report_individual.jsp?qFrom=<%= sdf2.format(q4From)%>&qTo=<%=sdf2.format(q4To)%>&userId=<%=userId%>&view=quarterly"><c:out value="${sq4}"/></a></td>
				</tr>
...
<%--					<td class="report" align="right"><b><a href="view_report_individual.jsp?qFrom=<%= sdf2.format(q1From)%>&qTo=<%=sdf2.format(q4To)%>&userId=<c:out value="${user.id}"/>&view=quarterly"><c:out value="${sqTotal}"/></a></b></td>--%>
					<td class="report" align="right"><b><a href="view_report_individual.jsp?qFrom=<%= sdf2.format(q1From)%>&qTo=<%=sdf2.format(q4To)%>&userId=<%=userId%>&view=quarterly"><c:out value="${sqTotal}"/></a></b></td>
				</tr>
		...
		<%
		// adding page break after two reports per page : reports of 2 account managers per page
		...
		<script>
			function openWindow(){
				window.open('printSalesReportIndividual.jsp');
			}
		</script>
		...
		<tr>
			<td colspan="2" >
				...
				<form name="printForm" action="" method="post">
					<div align="right"><input type="button" name="print" value="Print Report" onclick="openWindow();" class="button"></div>
				</form>
				</div>
			</td>
		</tr>
