...
<x:config>
    <page name="jsp_listingOpportunity_User">
     	<com.tms.crm.sales.ui.OpportunityListingTable name="table1" width="100%" />
    </page>
</x:config>

<x:set name="jsp_listingOpportunity_User.table1" property="accountManagerID" value="${currentUser.id}" />
...
<c:choose>
	<c:when test="${not empty(param.fromDate)}">
        ...
        %>
        <x:set name="jsp_listingOpportunity_User.table1" property="fromDate" value="${fromDate}" />
        <x:set name="jsp_listingOpportunity_User.table1" property="toDate"   value="${toDate}" />
	</c:when>

	<c:otherwise>
		<c:set var="fromDate" value="${widgets['jsp_listingOpportunity_User.table1'].fromDate}"/>
		<c:set var="toDate"   value="${widgets['jsp_listingOpportunity_User.table1'].toDate}"/>
	</c:otherwise>
...
<table cellpadding="4" cellspacing="1" class="sfaBackground" width="100%">
    ...
    <tr>
    <td class="sfaRow">
	<x:display name="jsp_listingOpportunity_User.table1"></x:display>
        </td>
    </tr>
</table>
