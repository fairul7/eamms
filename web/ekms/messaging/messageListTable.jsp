...
<c-rt:set var="DRAG_ENABLED" scope="request" value="<%= Application.getInstance().getProperty(MessagingModule.APPLICATION_KEY_DRAG_DROP_MESSAGES) %>"/>
<c:set var="dragKeyList" scope="request" value=""/>
<script type="text/javascript">
<!--
function toggleSelect(cbKey) {
    <c:if test="${DRAG_ENABLED}">
    var cb = document.getElementById(cbKey);
    var row = document.getElementById("row_" + cbKey);
    if (cb.checked) {
        row.className='menuBgBackground';
    }
    else {
            ...
            <table border="0" cellpadding="2" cellspacing="1" width="100%">
                ...
                <tbody>
                    ...
                    <c:forEach items="${model.tableRows}" var="row" varStatus="status">
                        ...
                        <tr class="tableRow" id="row_cb_<c:out value="${key}"/>">
                            <%-- Show Index Column --%>
                            <c:if test="${showIndex}">
                                <td valign="top" nowrap>
                                    <c:if test="${DRAG_ENABLED}">
                                        <c-rt:set var="msgPrefix" value="<%= MessageTable.MESSAGE_PREFIX %>"/>
                                        <span id="<c:out value="${msgPrefix}${key}"/>" style="position:relative"><img alt="<fmt:message key='messaging.label.dragToMove'/>" src="<c:url value="/ekms/images/ic_drag.gif"/>"></span>
                                        <c:if test="${status.index > 0}"><c:set var="dragKeyList" scope="request" value="${dragKeyList},"/></c:if>
                                        <c:set var="dragKeyList" scope="request" value="${dragKeyList}\"${msgPrefix}${key}\""/>
                                    </c:if>
                            ...
                            <%-- Show Data Columns --%>
                            <c:forEach items="${model.columnList}" var="column" varStatus="status">
...
<%--
                                    <c:if test="${DRAG_ENABLED && xproperty == 'indicator'}">
                                        <div title="<fmt:message key='messaging.label.dragToMove'/>" style="width:10px;position:relative" id="<c:out value="${key}"/>">
                                        <c:if test="${status.index > 0}"><c:set var="dragKeyList" scope="request" value="${dragKeyList},"/></c:if>
                                        <c:set var="dragKeyList" scope="request" value="${dragKeyList}\"${key}\""/>
                                    </c:if>
--%>
                                    <c:choose>
                                        ...
                                        <c:when test="${empty column.url}">
                                            ...
                                            <x:event name="${table.absoluteName}" type="sel" param="${column.urlParam}=${keyValue}&index=${idx}">
