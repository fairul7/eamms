...
<x:config>
     <page name="rateCardEdit">
        <com.tms.fms.facility.ui.RateCardEditForm name="form"/>
     </page>
</x:config>
...
<c:if test="${forward.name =='CATEGORY_COMPULSORY'}">
		<script>alert("<fmt:message key='fms.facility.msg.vehicleCompulsory'/>");</script>
</c:if>

<x:set name="rateCardEdit.form" property="action" value="continuesetup"/>
...
<table width="100%" cellpadding="0" cellspacing="0" class="contentBgColor">
	<tr valign="MIDDLE">
    	<td height="22" class="contentTitleFont">
      		&nbsp;<fmt:message key='general.label.systemAdministration'/> > <fmt:message key='fms.label.rateCardEdit'/>  
        </td>
  	...
  	<tr>
    	<td colspan="2" valign="TOP" class="contentBgColor">
			<x:display name="rateCardEdit" ></x:display>
		</td>
  	</tr>
  	<tr>
    	<td colspan="2" valign="TOP" class="contentBgColor">
