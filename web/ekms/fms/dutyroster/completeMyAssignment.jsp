...
<x:config>
     <page name="engineeringAssignment">
        <com.tms.fms.engineering.ui.CompleteAssignmentForm name="form"/>
     </page>
</x:config>
...
<c:choose>
  <c:when test="${forward.name == 'COMPLETE'}">
    <script>alert('ASSIGNMENT COMPLETED'); 
    document.location = "<c:url value="myEngineeringAssignment.jsp"/>";</script>
  </c:when>
  <c:when test="${forward.name == 'ATTACH'}">
    <script> 
    document.location = "<c:url value="completeMyAssignment.jsp?id=${id}"/>";</script>
  </c:when>
