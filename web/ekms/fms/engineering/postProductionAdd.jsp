...
<x:config>
	<page name="fms_postProductionPage">
    	<com.tms.fms.engineering.ui.PostProductionForm name="add" width="100%"/>
	</page>
</x:config>
<c:set var="type" value="Add"/>
<x:set name="fms_postProductionPage.add" property="type" value="${type}"/>
...
<x:set name="fms_postProductionPage.add" property="requestId" value="${requestId}"/>
<x:set name="fms_postProductionPage.add" property="dtRequiredFrom" value="${dtRequiredFrom}" />
<x:set name="fms_postProductionPage.add" property="dtRequiredTo" value="${dtRequiredTo}" />
...
<c:choose>
  ...
  <c:otherwise>
    <c:set var="serviceId" value="${widgets['fms_postProductionPage.add'].serviceId}"/>
  </c:otherwise>
</c:choose>
<x:set name="fms_postProductionPage.add" property="serviceId" value="${serviceId}"/>

<c:choose>
  <c:when test="${forward.name == 'ADDED'}">
	<script>
		alert('<fmt:message key="fms.facility.msg.postProductionBookingAdded"/>'); 
  ...
  <c:when test="${forward.name == 'FAILED'}">
    <script>alert('<fmt:message key="fms.facility.msg.postProductionBookingNotAdded"/>');</script>
  </c:when>
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
    ...
    <tr><td colspan="2" valign="TOP" bgcolor="#EFEFEF" class="contentBgColor">
        <x:display name="fms_postProductionPage.add"/>
    </td></tr>
</table>
<script>
var dateFrom=document.forms["fms_postProductionPage.add"].elements["fms_postProductionPage.add.requiredDateFrom"].value;
var dateTo=document.forms["fms_postProductionPage.add"].elements["fms_postProductionPage.add.requiredDateTo"].value;
var h1 = 0;
var h2 = 0;
var m1 = 0;
var m2 = 0;
