...
<x:config>
	<page name="fms_manpowerServicePage">
    	<com.tms.fms.engineering.ui.ManpowerServiceForm name="add" width="100%"/>
	</page>
</x:config>
<c:set var="type" value="Add"/>
<x:set name="fms_manpowerServicePage.add" property="type" value="${type}"/>
...
<x:set name="fms_manpowerServicePage.add" property="requestId" value="${requestId}"/>
<x:set name="fms_manpowerServicePage.add" property="dtRequiredFrom" value="${dtRequiredFrom}" />
<x:set name="fms_manpowerServicePage.add" property="dtRequiredTo" value="${dtRequiredTo}" />
...
<c:choose>
  ...
  <c:otherwise>
    <c:set var="serviceId" value="${widgets['fms_manpowerServicePage.add'].serviceId}"/>
  </c:otherwise>
</c:choose>
<x:set name="fms_manpowerServicePage.add" property="serviceId" value="${serviceId}"/>
...
<c:choose>
  <c:when test="${forward.name == 'ADDED'}">
	<script>
	    ...
	    document.location="manpowerServiceAdd.jsp?serviceId=<c:out value='${serviceId}' />";
    	<!--window.close()-->
    </script>
  </c:when>
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
    ...
    <tr><td colspan="2" valign="TOP" bgcolor="#EFEFEF" class="contentBgColor">
        <x:display name="fms_manpowerServicePage.add"/>
    </td></tr>
</table>

<script>
var dateFrom=document.forms["fms_manpowerServicePage.add"].elements["fms_manpowerServicePage.add.requiredFrom"].value;
var dateTo=document.forms["fms_manpowerServicePage.add"].elements["fms_manpowerServicePage.add.requiredTo"].value;
