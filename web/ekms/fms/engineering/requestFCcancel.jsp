...
<x:config>
     <page name="RequestFCcancel">
		<com.tms.fms.engineering.ui.RequestFCCancelForm name="form" width="100%"/>
     </page>
</x:config>

<c:set var="requestId" value="${widgets['fms_requestDetailsPage.details'].requestId}"/>
<x:set name="RequestFCcancel.form" property="requestId" value="${requestId}"/>
...
<c:choose>
  <c:when test="${forward.name == 'UPDATE'}">
   ...
   <c:when test="${forward.name == 'WARNING'}">
	<script>
		alert('<fmt:message key="fms.facility.msg.requestCancelledWarning"/>'); 
    </script>
  </c:when>
</c:choose>
...
<table width="100%" cellpadding="0" cellspacing="0" class="contentBgColor">
	<tr valign="MIDDLE">
    	<td height="22" class="contentTitleFont">
      		&nbsp;<fmt:message key='fms.request.label.requestCancel'/>  
        </td>
  	...
  	<tr>
    	<td colspan="2" valign="TOP" class="contentBgColor">
			<x:display name="RequestFCcancel.form" ></x:display>
		</td>
  	</tr>
  	<tr>
    	<td colspan="2" valign="TOP" class="contentBgColor">
