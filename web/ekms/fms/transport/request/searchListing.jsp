...
<x:config>
    <page name="transport">         
    	<com.tms.fms.transport.ui.SearchListing name="table" />        
    </page>
</x:config>

<c:if test="${!empty param.flagId}">
	<c:redirect url="assignment.jsp?flagId=${param.flagId}"/> 
</c:if>
...
<c:set var="bodyTitle" scope="request"><fmt:message key='fms.tran.assignmentSearch'/></c:set>
