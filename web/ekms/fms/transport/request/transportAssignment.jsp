 <%@include file="/common/header.jsp" %>

<x:config>
     <page name="request">
        <com.tms.fms.transport.ui.TranAssignmentForm name="assignmentform"/>
        <com.tms.fms.transport.ui.AssignmentTable name="assignmenttable"/>
     </page>
</x:config>


<c:if test="${! empty param.requestId}">
    <c:set var="requestId" value="${param.requestId}"></c:set>	
    <x:set name="request.assignmentform" property="id" value="${requestId}"/>
    <x:set name="request.assignmenttable" property="requestId" value="${requestId}"/>
</c:if>

<c:if test="${!empty param.id}">
	<c:redirect url="assignVehiclesDrivers.jsp?id=${param.id}"/>
</c:if>

<c:if test="${forward.name == 'Back'}" >  
  <c:redirect url="incomingListing.jsp"/>    
</c:if>

<c:if test="${forward.name == 'Approve'}" >
  <script>
  		alert('<fmt:message key="fms.tran.msg.requestApproved"/>');
  		document.location="pendingListing.jsp";	
   </script> 
</c:if>

<c:if test="${forward.name == 'Reject'}" >
  <script>
  		alert('<fmt:message key="fms.tran.msg.requestRejected"/>');
  		document.location="pendingListing.jsp";	
   </script> 
</c:if>

<c:if test="${forward.name == 'Edit'}" >  
  <c:redirect url="editRequest.jsp?id=${param.requestId}" />    
</c:if>

<c:if test="${forward.name == 'Outsource'}" >  
  <c:redirect url="outsource.jsp?id=${param.requestId}" />    
</c:if>
  
  
  
<%@include file="/ekms/includes/header.jsp" %>  
<jsp:include page="includes/header.jsp" />
<table width="100%" cellpadding="0" cellspacing="0" class="contentBgColor">
  <tr valign="MIDDLE">
	    <td height="22" class="contentTitleFont">
	      &nbsp;<fmt:message key='fms.tran.requestForTransport'/>  	      
	    </td>
	    <td align="right" class="contentTitleFont">&nbsp;</td>
	  	</tr>
	  	<tr>
	    	<td colspan="2" valign="TOP" class="contentBgColor">
	    <img src="<c:url value="/ekms/" />images/blank.gif" width="5" height="10"></td>
	  	</tr>
	  	<tr>
	    	<td colspan="2" valign="TOP" class="contentBgColor">
				<x:display name="request.assignmentform" ></x:display>
			</td>
  </tr>
  
<tr valign="MIDDLE">
    <td height="22" class="contentTitleFont">
      &nbsp;<fmt:message key='fms.tran.requestForTransport'/>      
    </td>
    <td align="right" class="contentTitleFont">&nbsp;</td>
  	</tr>
  	<tr>
    <td colspan="2" valign="TOP" class="contentBgColor">
    	<img src="<c:url value="/ekms/" />images/blank.gif" width="5" height="10"></td>
  	</tr>
  	<tr>
    	<td colspan="2" valign="TOP" class="contentBgColor">
			<x:display name="request.assignmenttable"> </x:display>
	</td>
</tr>
  

  
</table>
<jsp:include page="includes/footer.jsp" />



<%@include file="/ekms/includes/footer.jsp" %>