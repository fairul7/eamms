...
<x:config>
     <page name="request">
        <com.tms.fms.transport.ui.AssignAssignmentForm name="assignform"/>
...
<c:choose>
	<c:when test="${forward.name == 'vehicles'}">
			...
			<c:if test="${! empty assgId}" >  
			 <c:set var="assgId" value="${assgId}"/>		   
			</c:if>
...
<x:set name="request.check" property="assgId" value="${assgId}"/>
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
    ...
    <tr><td colspan="2" valign="TOP" bgcolor="#EFEFEF" class="contentBgColor">
        ...
        <x:display name="request.assignform" ></x:display>
