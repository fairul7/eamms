...
<c:if test="${forward.name == 'backToDetail'}" >
 	<script>
 		alert("Unable to submit, please check on the time and date!");
 		//document.location = "<c:url value="/ekms/fms/transport/request/"/>completeAssignment.jsp?id=<c:out value="${widgets['driverAssg.form'].id}"/>&userId=<c:out value="${widgets['driverAssg.form'].userId}"/>";
 	</script>
</c:if>
...
<c:if test="${forward.name == 'Ok'}" >
  ...
  <script>
  	...
  	document.location = "<c:url value="/ekms/fms/transport/request/"/>searchDriverAssignmentList.jsp";
  </script>
