...
<x:set name="Setup.form" property="setupType" value="<%= SetupObject.SETUP_CHANNEL %>"/>
<x:set name="Setup.table" property="setupType" value="<%= SetupObject.SETUP_CHANNEL %>"/>
    ...
    <x:set name="Setup.form" property="setupId" value="${setupID}"/>
  	<c:choose>
		<c:when test="${forward.name == forward_succ}">
			...
			<script>alert("<fmt:message key='fms.tran.msg.recordUpdated'/>"); document.location = "<c:url value="Channel.jsp"/>";</script>
		</c:when>
...
<c:choose>
	<c:when test="${forward.name == forward_listing_add}">
		<c:redirect url="Channel.jsp?action=add"/>
	</c:when>
...
<c:if test="${!empty param.setup_id}">
	<c:redirect url="Channel.jsp?action=edit&id=${param.setup_id}"/>
</c:if>

<x:set name="Setup.form" property="cancelUrl" value="Channel.jsp"/>
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
    <tr valign="middle">
        <td height="22" bgcolor="#003366" class="contentTitleFont"><b><font color="#FFCF63" class="contentTitleFont"><fmt:message key='fms.tran.setup.channelSetup'/></font></b></td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
<tr valign="middle">
    <td height="22" bgcolor="#003366" class="contentTitleFont"><b><font color="#FFCF63" class="contentTitleFont"><fmt:message key='fms.tran.setup.channelListing'/></font></b></td>
    <td align="right" bgcolor="#003366" class="contentTitleFont">&nbsp;</td>
</tr>
<tr><td colspan="2" valign="TOP" bgcolor="#EFEFEF" class="contentBgColor"><img src="<c:url value="/ekms/" />images/blank.gif" width="5" height="10"></td></tr>
<tr><td colspan="2" valign="TOP" bgcolor="#EFEFEF" class="contentBgColor">
