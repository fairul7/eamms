...
<c:choose>
  ...
  <c:otherwise>
    <c:set var="workingProfileId" value="${widgets['fms_workingProfilePage.edit'].workingProfileId}"/>
  </c:otherwise>
</c:choose>
...
<c:choose>
  ...
  <c:when test="${forward.name == 'defaultProfileEXISTS'}">
    <script>alert('<fmt:message key="fms.facility.msg.defaultWorkingProfileExists"/>');</script>
  </c:when>
