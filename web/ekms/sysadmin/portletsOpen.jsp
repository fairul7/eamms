...
<c:if test="${forward.name == forward_add_preference}">
    ...
    %>
    <script>
        window.open("<c:url value="portletPreferences.jsp?portletId=${pid}"/>", "preferenceWindow", "height=200,width=550,left=50,top=50,screenx=50,screeny=50,scrollbars=yes,resizable=yes");
    </script>
</c:if>
<c:if test="${(!(empty param.portletId)) && (!(empty param.name))}">
    <script>
        window.open("<c:url value="portletPreferences.jsp?portletId=${param.portletId}&name=${param.name}"/>", "preferenceWindow", "height=200,width=550,left=50,top=50,screenx=50,screeny=50,scrollbars=yes,resizable=yes");
    </script>
</c:if>
<%-- Render --%>
