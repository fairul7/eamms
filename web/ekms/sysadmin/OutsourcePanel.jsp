...
<x:permission permission="com.tms.fms.facility.permission.fmsAdmin" module="com.tms.fms.transport.model.TransportModule" url="/ekms/home.jsp"/>
...
<c-rt:set var="forward_name_invalid" value="<%= SetupForm.SETUP_ADD_NAME_INVALID %>"/>
...
<x:set name="Setup.form" property="setupType" value="<%= SetupObject.SETUP_OUTSOURCE_PANEL %>"/>
<x:set name="Setup.table" property="setupType" value="<%= SetupObject.SETUP_OUTSOURCE_PANEL %>"/>
...
<c:choose>
    ...
    <x:set name="Setup.form" property="setupId" value="${setupID}"/>
  	<c:choose>
		<c:when test="${forward.name == forward_succ}">
			...
			<script>alert("<fmt:message key='fms.tran.msg.recordUpdated'/>"); document.location = "<c:url value="OutsourcePanel.jsp"/>";</script>
		</c:when>
  ...
  <c:otherwise>
  	...
  	<c:choose>
		...
		<c:when test="${forward.name == forward_name_invalid}">
			<script>alert("<fmt:message key='fms.tran.msg.errorRecordNameIsEmpty'/>");</script>
		</c:when>
	</c:choose>
  </c:otherwise>
</c:choose>
...
<c:choose>
	<c:when test="${forward.name == forward_listing_add}">
		<c:redirect url="OutsourcePanel.jsp?action=add"/>
	</c:when>
...
<c:if test="${!empty param.setup_id}">
	<c:redirect url="OutsourcePanel.jsp?action=edit&id=${param.setup_id}"/>
</c:if>

<x:set name="Setup.form" property="cancelUrl" value="OutsourcePanel.jsp"/>
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
    <tr valign="middle">
        <td height="22" bgcolor="#003366" class="contentTitleFont"><b><font color="#FFCF63" class="contentTitleFont"><fmt:message key='fms.tran.setup.outsourcePanelSetup'/></font></b></td>
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
<tr valign="middle">
    <td height="22" bgcolor="#003366" class="contentTitleFont"><b><font color="#FFCF63" class="contentTitleFont"><fmt:message key='fms.tran.setup.outsourcePanelListing'/></font></b></td>
    <td align="right" bgcolor="#003366" class="contentTitleFont">&nbsp;</td>
</tr>
<tr><td colspan="2" valign="TOP" bgcolor="#EFEFEF" class="contentBgColor"><img src="<c:url value="/ekms/" />images/blank.gif" width="5" height="10"></td></tr>
<tr><td colspan="2" valign="TOP" bgcolor="#EFEFEF" class="contentBgColor">
