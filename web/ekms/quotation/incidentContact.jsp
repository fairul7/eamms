...
<c:if test="${not empty param.clear}" >
    <x:set name="quotationContact.table1" property="clear" value="1" />
</c:if>

<c:if test="${not empty param.companyId}" >
    <x:set name="quotationContact.table1" property="companyID" value="${param.companyId}" />
    <x:set name="quotationContact.form1" property="companyID" value="${param.companyId}" />
</c:if>
...
<c:if test="${forward.name == 'contactAdded'}" >
    <c:set var="table" value="${widgets['quotationContact.table1']}"/>
    <c:set var="contactId" value="${widgets['quotationContact.form1'].justCreatedID}"/>
    <%
...
<c:if test="${forward.name == 'contactsSelected' }" >
    <c:set var="companyId" value="${widgets['quotationContact.table1'].companyID}" />
    <c:set var="contactList" value="${widgets['quotationContact.table1'].contactList}"/>
    <%
    ...
    %>
    <c:redirect url="addQuotation.jsp?companyId=${companyId}&contactId=${contactId}" />
</c:if>
...
<table cellpadding="4" cellspacing="1" class="sfaBackground" width="100%">
	...
	<x:display name="quotationContact.table1"></x:display>
    </td>
    </tr>
   ...
   <table cellpadding="4" cellspacing="1" class="sfaBackground" width="100%">
    ...
    <td class="sfaHeader">
     ...
     <fmt:message key='sfa.message.newContact'/>
        ...
        <tr>
        <td class="sfaRow">
	<x:display name="quotationContact.form1"></x:display>
       </td>
       </tr>
    </table>
