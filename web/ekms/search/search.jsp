...
<x:template type="com.tms.ekms.search.ui.SearchProfileProcessAdd" properties="forward=../search/searchProfile.jsp" />
<%-- Event Handling --%>
...
<c:if test="${param.et == searchEvent}">
    <c:choose>
    	<c:when test="${param[searchModule] == 'com.tms.cms.medialib.model.MediaModule'}">
    		<c:redirect url="/ekms/medialib/editMedia.jsp?id=${param[searchKey]}"/>
    	</c:when>
        ...
        <c:when test="${param[searchModule] == 'com.tms.collab.myfolder.model.MyFolderModule'}">
        	<c:redirect url="/ekms/myfolder/viewSearch.jsp?id=${param[searchKey]}"/>
        </c:when>
    </c:choose>
</c:if>
<%-- END: Event Handling --%>
...
<x:template type="com.tms.ekms.search.ui.SearchWidget" properties="pageSize=20" />
