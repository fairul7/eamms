...
<x:permission module="<%= DirectoryModule.class.getName() %>" permission="<%= DirectoryModule.PERMISSION_EDIT_CONTACTS %>" url="noPermission.jsp" />

<x:config>
    <page name="bdImportContact">
        <com.tms.collab.directory.ui.DirectoryContactImportForm name="form" />
    </page>
</x:config>
...
<c:if test="${!empty param.id}">
    <x:set name="bdImportContact.form" property="id" value="${param.id}"/>
</c:if>

<c:if test="${!empty param['button*bdImportContact.form.buttonPanel.cancel_form_action']}">
    <c:redirect url="abViewContact.jsp?id=${widgets['bdImportContact.form'].id}" />
</c:if>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr valign="MIDDLE">
    <td height="22" bgcolor="#003366" class="contentTitleFont"><b><font color="#FFCF63" class="contentTitleFont">
      &nbsp;<fmt:message key='addressbook.label.businessDirectoryImportContact'/>
    </font></b></td>
  ...
  <tr>
    <td colspan="2" valign="TOP">
        <x:display name="bdImportContact.form" />
    </td>
  </tr>
  <tr>
