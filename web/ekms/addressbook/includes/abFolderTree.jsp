...
<x:config>
    <page name="abFolderTree">
        <com.tms.collab.directory.ui.FolderTree name="tree" />
    </page>
</x:config>

<x:display name="abFolderTree.tree" body="custom">
    ...
    <table width="10" align="left"><tr><td></td></tr></table>
    ...
    <tr>
        <td valign=top>
            <c:if test="${!empty root[tree.childrenProperty] && !empty root[tree.childrenProperty][0]}">
                <a class="contentTreeNode" href="#" onclick="treeToggle('ab<c:out value="${tree.name}"/><c:out value="${root[tree.displayId]}"/>'); return false"><span id="icon_ab<c:out value="${tree.name}"/><c:out value="${root[tree.displayId]}"/>">+</span></a>
            </c:if>
            <c:if test="${empty root[tree.childrenProperty] || empty root[tree.childrenProperty][0]}">
        ...
        <td>
            ...
            <c:if test="${!empty root[tree.childrenProperty] && !empty root[tree.childrenProperty][0]}">
                <span id="ab<c:out value="${tree.name}"/><c:out value="${root[tree.displayId]}"/>" style="display: block">
                    <script>
                    <!--
                        treeLoad('ab<c:out value="${tree.name}"/><c:out value="${root[tree.displayId]}"/>');
                    //-->
                    </script>
