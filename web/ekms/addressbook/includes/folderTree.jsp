...
<tr>
    <td valign=top>
        <c:if test="${!empty root[tree.childrenProperty] && !empty root[tree.childrenProperty][0]}">
            <a class="contentTreeNode" href="#" onclick="treeToggle('<c:out value="${tree.name}"/><c:out value="${root[tree.displayId]}"/>'); return false"><span id="icon_<c:out value="${tree.name}"/><c:out value="${root[tree.displayId]}"/>">-</span></a>
        </c:if>
        <c:if test="${empty root[tree.childrenProperty] || empty root[tree.childrenProperty][0]}">
