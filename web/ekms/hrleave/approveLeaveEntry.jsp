...
<c:choose>
    <c:when test="${forward.name == 'approve'}">
        <script>
            ...
            document.location = "<c:url value="/ekms/hrleave/viewApproveList.jsp" />";
        </script>
    </c:when>
...
<c:choose>
    <c:when test="${!empty param.entryId}">
        <x:set name="approveLeaveEntry.form" property="id" value="${param.entryId}" />
    </c:when>
</c:choose>
