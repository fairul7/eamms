...
<x:config>
    <page name="meetingActionItemsPage">
        <com.tms.collab.emeeting.ui.MeetingActionItemsForm name="actionItemsForm"/>
    </page>
</x:config>
...
<c:if test="${forward.name == 'refresh'}" >
    ...
    <script>
        location = '<%=response.encodeURL(request.getRequestURI())%>';
        window.opener.location.reload();
    </script>
</c:if>
<c-rt:set var="forward_assign" value="<%= MeetingActionItemsForm.FORWARD_ASSIGN_TASK%>"/>
<c-rt:set var="forward_edit" value="<%= MeetingActionItemsForm.FORWARD_EDIT_TASK%>"/>
...
<c:if test="${! empty selectedItem}">
    <c:if test="${forward.name == forward_assign}">
        <script>window.open("<c:url value="/ekms/calendar/"/>emeetingTask.jsp?itemId=<c:out value="${selectedItem.itemId}"/>&taskId=-1", "emeetingTask", "height=400,width=550,left=50,top=50,screenx=50,screeny=50,scrollbars=yes,resizable=yes");</script>
    </c:if>
    <c:if test="${forward.name == forward_edit}">
        <script>window.open("<c:url value="/ekms/calendar/"/>emeetingTask.jsp?itemId=<c:out value="${selectedItem.itemId}"/>&taskId=<c:out value="${selectedItem.item.taskId}"/>", "emeetingTask", "height=400,width=550,left=50,top=50,screenx=50,screeny=50,scrollbars=yes,resizable=yes");</script>
    </c:if>
</c:if>
<html>
<head>
