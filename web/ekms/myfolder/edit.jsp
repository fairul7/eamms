...
<c:if test="${forward.name == 'cancel'}">
    <script>
        //		window.location = "view.jsp?id=<c:out value='${param.id}' />";
        history.go(-2);
    </script>
</c:if>

<c:set var="bodyTitle" scope="request"><fmt:message key="mf.label.myFolder"/> > <fmt:message key="mf.label.edit"/></c:set>
