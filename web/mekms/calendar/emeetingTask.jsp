<%@ page import="com.tms.collab.emeeting.ui.MMeetingTaskForm,
				 com.tms.collab.emeeting.ui.MeetingTaskForm,
...
<x:config>
    <page name="emeetingTask">
        <com.tms.collab.emeeting.ui.MMeetingTaskForm name="taskForm"/>
    </page>
</x:config>
...
<x:set name="emeetingTask.taskForm" property="mobile" value="${true}"/>
<%-- Event Handling --%>
...
<c:if test="${forward.name == addresource}" >
 <%
     ...
     MMeetingTaskForm form = null;
     if(wm!=null){
         form = (MMeetingTaskForm)wm.getWidget("emeetingTask.taskForm");
         if(form!=null)
...
//         window.opener.location.reload();
        document.location = "<c:url value="/mekms/calendar/taskresourcepop.jsp?taskId=${taskId}" ></c:url>"
         </script>
</c:if>
