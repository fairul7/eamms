...
<x:config >
    <page name="conflictpage">
        <com.tms.collab.calendar.ui.ConflictForm name="conflictform" template="calendar/mconflictsform"/>
    </page>
</x:config>
...
<c:choose>
    <c:when test="${forward.name == 'added'}" >
        <c:set var="form" value="${widgets['calendarformpage.AppointmentForm']}"  />
        <%
            MCalendarForm form = (MCalendarForm) pageContext.getAttribute("form");
            form.init();
        %>
        <script>
            <%
            ...
            document.location = "<c:url value="/mekms/calendar/calendar.jsp"/>?cn=calendarPage.calendarView&et=select&eventId=<%=(cview.getEvent().getEventId())%>";
        </script>
    </c:when>
    ...
    <c:when test="${forward.name == 'cancel'}" >
        ...
        <c:when test="${id == appointment}" >
            <c:redirect url="/mekms/calendar/calendarform.jsp" ></c:redirect>
        </c:when>
        <c:when test="${id == event}" >
            <c:redirect url="/mekms/calendar/eventform.jsp" ></c:redirect>
        </c:when>
        <c:when test="${id == emeeting}" >
            <c:redirect url="/mekms/calendar/emeetingform.jsp" ></c:redirect>
        </c:when>
        </c:choose>
    </c:when>
