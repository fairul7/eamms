...
<x:config >
    <page name="progressformpage" >
        <com.tms.collab.taskmanager.ui.ProgressForm name="progressform" template="taskmanager/mprogressform"/>
    </page>
</x:config>
...
<c:if test="${forward.name == set}" >
    <script>
        ...
        window.opener.location = "<c:url value="/mekms/calendar/calendar.jsp?taskId=setProgress.Task_1" ></c:url>";
        window.close();
    </script>
</c:if>
