...
function getCaretEnd(obj){
    //- MODIFIED: to support Opera and textarea in IE
    if(is_opera) {
        return obj.value.length;
    }else if(typeof obj.selectionEnd != "undefined"){
		...
		var rb = obj.value.length + 1;
		var Lp = document.selection.createRange().duplicate();
		while ( Lp.parentElement() == obj &&
		    Lp.move("character",1) == 1 ) {
		    --rb;
		}
		if ( rb == obj.value.length + 1 ) {
		    rb = -1;
		}
		return rb;
	     }
...
function actb(obj,evt,ca){
	...
	/* --- Styles --- */
	var actb_bgColor = '#BAD9FA';
	var actb_textColor = 'black';
	var actb_hColor = 'white';
	...
	// MODIFIED: event handling to support ie and gecko
	document.onkeydown = actb_keydown;
	...
	function actb_generate(){
		...
		var a = document.createElement('table'); // MODIFIED: Added var to prevent exception due to richtextbox
		a.cellSpacing='1px';
		a.cellPadding='2px';
		...
		if (actb_delimiter.length > 0){
			...
			for (i=0;i<actb_delimwords.length;i++){
				if (actb_cdelimword == i){
				    if (str.length > 0) str += ' '; // MODIFIED: To add a space before inserted word
					str += a;
					str += ","; // MODIFIED: To add a delimiter after inserted word
					l = str.length;
				}else{
	...
	function actb_penter(){
        ...
        //- MODIFIED: return false to not move to a new line
        return false;
	}
...
//		return !actb_caretmove;
	...
	function actb_checkkey(evt){
	  try {
		if (!evt) evt = event;
		var a = evt.keyCode; // MODIFIED: Added var to prevent exception due to richtextbox
		...
		switch (a){
			...
			case 40:
				...
				break;
			case 13: case 9:
                //- MODIFIED: return value to not move to a new line if a word is inserted
				actb_caretmove = 1;
                return actb_penter();
			default:
				setTimeout(function(){actb_tocomplete(a)},50);
//				setTimeout(function(){try { actb_tocomplete(a) } catch(e) {} },50); // MODIFIED: catch exception when e.g. Ctrl-R
				break;
		}
	  }
	  catch(e) {
